...
def test_fetch_worklog_sources_batches_base_tags_and_direct_predecessors() -> None:
    ...
    sources = asyncio.run(
        GraphWorklogSourceStore().fetch_worklog_sources(
            session=session,
    ...
    assert sources[101].tags == [
        GraphWorklogTag(tag_id=11, tag_name="정산", description="정산 배치와 대사 업무")
    ]
    assert sources[101].direct_predecessors == [
        GraphWorklogPredecessor(worklog_id=88, title="직접 선행")
    ]
