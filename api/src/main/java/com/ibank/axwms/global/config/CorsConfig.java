...
/**
 * refresh 쿠키 주고받기와 accessToken Authorization 헤더 노출을 위해 CORS 정책을 구성하는 설정 클래스.
 * 허용 오리진은 CorsProperties 로만 조정하고, credentials 허용 여부·노출 헤더는 보안 정책상 여기 하드코딩한다.
 */
@Configuration(proxyBeanMethods = false)
public class CorsConfig {

    /** AuthTokenResponseWriter 가 내려주는 Authorization 응답 헤더를 브라우저 JS 에서 읽을 수 있도록 expose 한다. */
    @Bean
