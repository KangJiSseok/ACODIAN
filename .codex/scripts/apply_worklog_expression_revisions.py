...
def main():
    parser = argparse.ArgumentParser(description='Apply expression-level worklog revisions back into SQL.')
    ...
    for item in revisions:
        by_worklog.setdefault(int(item['worklog_id']), []).append(item)
    ...
    for line in sql_path.read_text(encoding='utf-8').splitlines():
        ...
        revisions_for_row = by_worklog.get(worklog_id)
        if not revisions_for_row:
            lines.append(line)
            continue
        work_content = unquote(vals[5]) or ''
        for rev in revisions_for_row:
            old = rev['old_sentence']
            new = rev['new_sentence']
            if old not in work_content:
                raise ValueError(f"Expression not found for worklog {worklog_id}: {old}")
            work_content = work_content.replace(old, new, 1)
            changed += 1
        vals[5] = quote(work_content)
